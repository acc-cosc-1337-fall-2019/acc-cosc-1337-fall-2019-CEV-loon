...
{
	double_angle = get_degrees(degrees - rotate_circle_offset);
	int x1 = width / 2;
	int y1 = width / 2;
	int x2 = cos(angle) * length*width / 2;
	int yx = sin(angle) * length*width / 2;

	std::make_unique<shape> line = std::make_unique<line>(device_context,
...
{
	double minutes_angle = clock.get_minutes() * 6 - rotate_circle_offset;
...
void ClockGraphic::draw_seconds_hand()
{
	...
	device_context->SetPen(wxPen(wxColor(124, 162, 0, ), 3))
...
void ClockGraphic::draw_hours_text()
{
	...
	std::make_unique<shape> text = std::make_unique<text>(device_context,
		std::to_string(hour), point(x, y));
	text->draw();
	}
}
...
void ClockGraphic::draw_seconds_markers()
{
	for (int i = 0; i < 60; ++i)
	{
		double sweconds_angle = i * 6 - rotate_circle_offset;
